...
module Scrabble =
    ...
    let playGame cstream pieces (st: state) =
        let rec aux (st: state) =
            ...
            | RCM(CMChangeSuccess(newPieces)) ->
                ...
                let updatedHand =
                    List.fold (fun acc (x, _) -> MultiSet.addSingle x acc) MultiSet.empty newPieces
    ...
    let startGame
        ...
        fun () ->
            playGame cstream tiles (State.mkState board dict playerNumber handSet Map.empty numPlayers turnCounter 100u)
