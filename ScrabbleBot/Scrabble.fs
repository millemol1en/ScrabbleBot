...
module State =
    ...
    let insertMovesIntoState (ms: (coord * (uint32 * (char * int))) list) (pob: Map<coord, (char * int)>) =
        let rec aux (moves: (coord * (uint32 * (char * int))) list) (acc: Map<coord, (char * int)>) =
            match moves with
            | [] -> acc
            | (coor, (_, (cVal, pVal))) :: xs -> aux xs (acc |> Map.add coor (cVal, pVal))
    ...
    let sendMoveToServer (cstream: Stream) (moveToPlay: (coord * (uint32 * (char * int))) list) =
        ...
        printfn "Number of moves to play: %d" moveToPlay.Length

        // Print each move in the list
        moveToPlay |> List.iter (fun move -> printfn "Move: %A" move)
        ...
        if moveToPlay.IsEmpty then
            ...
            // else
            printfn "No moves to play. Passing turn."
            send cstream (SMPass)
        else
            printfn "Sending moves to server."
...
module Scrabble =
        ...
        let rec aux (st: state) =
            ...
            if (st.turnCounter = st.playerNumber) then
                ...
                if (isBoardEmpty st) then
                    ...
                    let longestParsedWord =
                        parseBotMove st (longestWordOnFirstTurn, ((st.board).center, Horizontal))

                    printParseMove (longestParsedWord)

                    printf "\n\nLength off first move :: %s\n" (longestWordOnFirstTurn)
                    ...
                    printfn "Longest word we can play: %s" (fst longestWord)
                    printfn "Longest word coordinate and direction: %A" (snd longestWord)

                    // // Print all words we could play
                    // printfn "All words we could play:"
                    // printAllWordsWeCouldPlay (collectAllTheWordsWeCanPlay st)
                    ...
                    printfn "Length of move: %s" (fst longestWord)
    ...
    let startGame
        ...
        fun () ->
            playGame cstream tiles (State.mkState board dict playerNumber handSet Map.empty numPlayers turnCounter)
