...
let parseBotMove
    (st: state)
    ((s, (c, d)): string * (Coordinate * Direction))
...
let listOfAllWordsWeCanPlay (st: state) =
    List.fold
        (fun (acc: (string * (Coordinate * Direction)) list) (key, value) ->
            let (coord, dir) = value
...
let longestWordWeCanPlay (st: state) =
    List.fold
        (fun (accWord: (string * (Coordinate * Direction))) (currWord: string * (Coordinate * Direction)) ->
...
//     let checkWordAsync (currWord: string * (Coordinate * Direction)): Async<string * (Coordinate * Direction)> = async {
//         let move = parseBotMove st currWord
//         let tempSt = createTempState st move
//         let tempBoard = gatherWordsOnTheBoard tempSt
//         let! isValid = isWordValidAsync tempBoard st currWord
//         return if isValid then currWord else ("", ((0, 0), Center))
//     }

//     let longestWordTask = words |> List.map checkWordAsync |> Async.Parallel
//     let longestWordResult = longestWordTask |> Async.RunSynchronously
//     longestWordResult
//     |> Array.fold (fun (accWord: string * (Coordinate * Direction)) (currWord: string * (Coordinate * Direction)) ->
//         if (fst currWord).Length > (fst accWord).Length then currWord else accWord
//     ) ("", ((0, 0), Center))
